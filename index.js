...
app.post('/createpay', async (req, res) => {
    try {
        ...
        const apiUrl = `https://api-linecloud.digital-server.biz.id/api/pay/okt-deposit`;
        const params = {
            amount: amount,
            qrcode: YOUR_QR_CODE,
            apikey: KeyApi
...
app.get('/cekstatus', async (req, res) => {
    try {
        const apiUrl = `https://api-linecloud.digital-server.biz.id/api/pay/okt-status`;
        const params = {
            merchant: YOUR_MEMID,
...
app.post('/create-server', async (req, res) => {
    ...
    try {
        const response = await fetch(`https://api-linecloud.digital-server.biz.id/api/panel/ptero?domain=${domain}&apikey=${apikey}&username=${username}&ram=${ram}&disk=${disk}&cpu=${cpu}`, {
            method: "GET",
        ...
        if (!data.server_id) {
            return res.status(500).json({ message: `Error: ${data.message || "Terjadi kesalahan saat membuat server."}` });
        }
...
<body>
    <div class="container">
        ...
        <a href="${data.login_url}" class="login-btn" target="_blank">
        ...
        <div class="warning-note">
            ...
            Simpan informasi ini dengan aman. Password tidak dapat diambil kembali.
        </div>

        <div style="margin-top: 30px; color: #718096; font-size: 14px;">
            <p>Butuh bantuan? <a href="mailto:wiraliwirya222210@gmail.com" style="color: #4299e1; text-decoration: none;">Hubungi Support</a></p>
            <p>© 2024 Wira Liwirya. All rights reserved.</p>
        </div>
    </div>
</body>
</html>
